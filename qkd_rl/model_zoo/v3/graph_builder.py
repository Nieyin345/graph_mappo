...
class GraphObservation:
    ...
    # Union of every shortest BFS route for currently pending requests in the
    # dynamic message graph. This is policy context only: edges outside the
    # mask remain valid generation candidates for future inventory.
    request_path_edge_mask: list[bool] | None = None
    # One shortest-path edge mask per GS pair, aligned with demand_edge_ids.
    # Inventory-only edges participate in context but never become actions.
    pair_path_edge_masks: list[list[bool]] = field(default_factory=list)
    pair_path_weights: list[float] = field(default_factory=list)
    # v3.1: current-request pairs define stage-one eligible arcs; historical
    # demand and inventory shortfall provide a stage-two preparation prior.
    pair_current: list[bool] = field(default_factory=list)
    preparation_edge_values: list[float] = field(default_factory=list)
...
class GraphBuilder:
    def __init__(
        ...
        # v3.1 preparation evidence, updated once per environment time step.
        # These are causal observations from past activations and visible
        # request/stock gaps; neither counter uses future demand.
        self._v31_history_t: int | None = None
        self._v31_edge_use = {edge.edge_id: 0.0 for edge in edges}
        self._v31_edge_shortage = {edge.edge_id: 0.0 for edge in edges}
    ...
    def build(
        ...
        request_path_edge_mask = self._request_shortest_path_union(
            active_edges, requests.get_pending()
        )
        path_cfg = self.config["model"]["actor"].get("pair_path_attention", {})
        max_hops = path_cfg.get("max_hops")
        pair_path_edge_masks = self._pair_shortest_path_masks(
            active_edges, demand_pairs,
            max_hops=int(max_hops) if max_hops is not None else None,
        )
        if max_hops is not None and env_state.t != self._v31_history_t:
            if self._v31_history_t is None or env_state.t < self._v31_history_t:
                self._v31_edge_use = {edge.edge_id: 0.0 for edge in self.edges}
                self._v31_edge_shortage = {edge.edge_id: 0.0 for edge in self.edges}
                elapsed = 1
            else:
                elapsed = max(1, env_state.t - self._v31_history_t)
            decay = 0.99 ** elapsed
            for edge_id in self._v31_edge_use:
                self._v31_edge_use[edge_id] *= decay
                self._v31_edge_shortage[edge_id] *= decay
            for edge_id in env_state.last_activated_edges:
                if edge_id in self._v31_edge_use:
                    self._v31_edge_use[edge_id] += 1.0
            for pair_pos, pair in enumerate(demand_pairs):
                stats = (demand_stats or {}).get(pair)
                pending = float(stats.pending_amount) if stats is not None else 0.0
                if pending <= 0:
                    continue
                for edge_pos, on_path in enumerate(pair_path_edge_masks[pair_pos]):
                    if on_path:
                        edge_id = active_edges[edge_pos].edge_id
                        if self.qkp.get_level(edge_id) < min(pending, self.qkp.get_capacity(edge_id) * 0.1):
                            self._v31_edge_shortage[edge_id] += 1.0
            self._v31_history_t = env_state.t
        pair_path_weights = []
        pair_current = []
        preparation_edge_values = [0.0] * len(active_edges) if max_hops is not None else []
        for pair in demand_pairs:
            stats = (demand_stats or {}).get(pair)
            pending = float(stats.pending_amount) if stats is not None else 0.0
            recent = request_history.sum(pair, "arrived", env_state.t, 240) / 240.0
            served = request_history.sum(pair, "served", env_state.t, 240) / 240.0
            pair_current.append(pending > 0)
            # Present demand dominates; historical demand is a small planning prior.
            pair_path_weights.append(
                pending / (pending + 1_000_000.0)
                + 0.1 * recent / (recent + 1000.0)
            )
            if max_hops is not None:
                pair_pos = len(pair_path_weights) - 1
                usage = served / (served + 1000.0)
                arrivals = recent / (recent + 1000.0)
                for edge_pos, on_path in enumerate(pair_path_edge_masks[pair_pos]):
                    if on_path:
                        edge = active_edges[edge_pos]
                        capacity = max(1.0, float(self.qkp.get_capacity(edge.edge_id)))
                        stock_gap = 1.0 - min(1.0, float(self.qkp.get_level(edge.edge_id)) / capacity)
                        repeated_use = self._v31_edge_use[edge.edge_id] / (self._v31_edge_use[edge.edge_id] + 10.0)
                        shortages = self._v31_edge_shortage[edge.edge_id] / (self._v31_edge_shortage[edge.edge_id] + 10.0)
                        preparation_edge_values[edge_pos] += (
                            0.25 * usage + 0.20 * arrivals + 0.20 * repeated_use
                            + 0.20 * shortages + 0.15 * arrivals * stock_gap
                        )
        if max_hops is not None and preparation_edge_values:
            peak = max(1.0, max(preparation_edge_values))
            preparation_edge_values = [value / peak for value in preparation_edge_values]
        action_candidates: dict[str, list[str]] = {}
        action_masks: dict[str, list[bool]] = {}
        ...
        return GraphObservation(
            ...
            request_path_edge_mask=request_path_edge_mask.tolist(),
            pair_path_edge_masks=pair_path_edge_masks,
            pair_path_weights=pair_path_weights,
            pair_current=pair_current,
            preparation_edge_values=preparation_edge_values,
        )

    def _pair_shortest_path_masks(
        self, edges: list[Edge], pairs: list[tuple[str, str]], max_hops: int | None = None
    ) -> list[list[bool]]:
        """Edges on shortest routes, or within a bounded source-to-target walk."""
        if not pairs:
            return []
        if max_hops is not None:
            if max_hops < 1:
                raise ValueError("pair_path_attention.max_hops must be positive")
            adjacency: dict[str, list[str]] = {nid: [] for nid in self.node_index}
            for edge in edges:
                adjacency[edge.src].append(edge.dst)
                adjacency[edge.dst].append(edge.src)
            distance_cache: dict[str, dict[str, int]] = {}

            def bounded_distances(start: str) -> dict[str, int]:
            ...
            for src, dst in pairs:
                from_src = bounded_distances(src)
                from_dst = bounded_distances(dst)
                masks.append([
                    min(
                        from_src.get(edge.src, max_hops) + 1 + from_dst.get(edge.dst, max_hops),
                        from_src.get(edge.dst, max_hops) + 1 + from_dst.get(edge.src, max_hops),
                    ) <= max_hops
                    for edge in edges
                ])
            return masks
        ...
        masks = []
        for src, dst in pairs:
            from_src, from_dst = distances(src), distances(dst)
            shortest = from_src.get(dst)
            if shortest is None:
                masks.append([False] * len(edges))
                continue
            masks.append([
                (from_src.get(edge.src, -10000) + 1 + from_dst.get(edge.dst, -10000) == shortest)
                or (from_src.get(edge.dst, -10000) + 1 + from_dst.get(edge.src, -10000) == shortest)
                for edge in edges
            ])
        return masks

    def _request_shortest_path_union(self, edges: list[Edge], pending_requests) -> np.ndarray:
        """Mark edges on any shortest path for any pending request.

        Marking all shortest paths avoids an arbitrary BFS tie-break. The
        result guides attention and never removes an action candidate.
        """
        mask = np.zeros(len(edges), dtype=bool)
        if not edges or not pending_requests:
            return mask
        adjacency: dict[str, list[tuple[str, int]]] = {nid: [] for nid in self.node_index}
        for pos, edge in enumerate(edges):
            adjacency[edge.src].append((edge.dst, pos))
            adjacency[edge.dst].append((edge.src, pos))
        ...
        for request in pending_requests:
            src = getattr(request, "src_gs", None)
            dst = getattr(request, "dst_gs", None)
            ...
            if shortest is None:
                continue
            for pos, edge in enumerate(edges):
                forward = from_src.get(edge.src, -1) + 1 + from_dst.get(edge.dst, -1)
                reverse = from_src.get(edge.dst, -1) + 1 + from_dst.get(edge.src, -1)
                if forward == shortest or reverse == shortest:
                    mask[pos] = True
        return mask
    ...
    def build_demand_pairs(
        ...
        if mode == "active_or_history_pairs":
            active = set(requests.demand_by_pair())
            historical = request_history.observed_pairs("arrived") - active
            max_history = int(cfg.get("max_history_pairs", 0) or 0)
            if max_history > 0 and len(historical) > max_history:
                windows = [int(w) for w in cfg.get("history_windows", [15, 60, 240])]
                total = request_history.total_all("arrived")
                reference = float(cfg.get("history_rate_reference", 1000.0))

                def strength(pair: tuple[str, str]) -> float:
                    recent = sum(
                        request_history.sum(pair, "arrived", env_state.t, w) / w
                        for w in windows
                    )
                    lifetime_share = request_history.total(pair, "arrived") / total if total > 0 else 0.0
                    return recent + reference * lifetime_share

                historical = set(sorted(historical, key=lambda pair: (-strength(pair), pair))[:max_history])
            return sorted(active | historical)
        ...
        for r, pair in enumerate(demand_pairs):
            ...
            if cfg.get("include_wait_bucket_amounts", False):
                for amount in stats.wait_bucket_amounts:
                    ...
                    col += 1
            if cfg.get("include_pair_history", False):
                windows = [int(w) for w in cfg.get("history_windows", [15, 60, 240])]
                rate_reference = float(cfg.get("history_rate_reference", 1000.0))
                if rate_reference <= 0.0:
                    raise ValueError("demand_edge.history_rate_reference must be positive")
                for kind in ("arrived", "served"):
                    for window in windows:
                        rate = max(0.0, request_history.sum(pair, kind, env_state.t, window)) / window
                        rows[r, col] = math.log1p(rate) / math.log1p(rate_reference)
                        col += 1
                total_arrived = request_history.total_all("arrived")
                rows[r, col] = (
                    request_history.total(pair, "arrived") / total_arrived
                    if total_arrived > 0.0 else 0.0
                )
                col += 1
            if col != demand_dim:
