...
from qkd_rl.rl.algos.checkpoint_compat import (
    _reset_module,
...
class UpdateStats:
    ...
    #    这里 ratio 是**整条匹配序列这个联合动作**的概率比：匹配策略是自回归
    #    分布，联合 log-prob 必须把各条件 log-prob 求和。旧实现做逐决策平均时，
    #    ratio 实际是联合 ratio 的 D 次方根，会把 clip/KL 人为压小。
    #    恒 0 ⟹ 近端约束没起作用；显著 >0 ⟹ 联合动作正在被 clip 截断，
    #    越大说明整条匹配序列的策略步长越激进。
    #
        ...
        for _epoch in range(epochs):
            ...
            for batch in batch_iter:
                ...
                if self.clip_per_role:
                    ...
                    #
                    # A single global clip historically let the critic crowd out
                    # the actor, so the two parameter roles stay clipped
                    # independently. Do NOT reuse the old gradient magnitudes as
                    # a current scale claim: after the matching probability fix,
                    # the actor optimizes the JOINT sequence log-prob (sum of
                    # conditional log-probs), not a per-decision mean. Bounded
                    # server smokes therefore show much larger raw actor norms;
                    # the calibrated joint-PPO preset lowers actor_lr instead of
                    # reverting to the mathematically wrong averaged ratio.
        ...
        for start in range(0, len(batch), chunk_size):
            ...
            for step, adv, (log_probs, entropies, value) in zip(chunk, chunk_advantages, batched_results):
                ...
                if node_ids:
                    ...
                    #
                    # Both log probabilities are SUMS over the stored ordered
                    # sequence, including STOP when sampled. The historical
                    # mean_log_prob field name does not imply normalization.
                    # Entropy is a separate per-decision regularizer.
