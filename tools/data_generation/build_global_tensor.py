...
from legacy_data import (gs, hap, sat, system,
                         GlobalGroundStations, GlobalHAPs, GlobalSatellites, SimConfig)
from legacy_physics import GeoMath, HapMobilityModel, R
...
def load_weather_data(gnodes, T_YEAR, THETA):
    ...
    for idx, node in enumerate(gnodes):
        ...
        missing_columns = []
        if cfg:
            file_path = os.path.join(_PROJECT_ROOT, "weather", "2023", f"{node.tag}_weather.csv")
