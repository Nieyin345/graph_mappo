...
class GeoMath:
    ...
    def is_line_of_sight_visible(node1, node2, t):
        ...
        """
        from legacy_data import gs, hap, sat  # 局部导入防止循环依赖
...
class HapMobilityModel:
    ...
    def update_hap_coordinates(method, hnodes, syst):
        ...
        file_name = os.path.join(project_root, "weather", f"wind_{level}hpa_era5.nc")
        ...
        if method == "wind":
            ...
            project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
            df = pd.read_csv(os.path.join(project_root, "dataset", "balloon_sim_data.csv"))
