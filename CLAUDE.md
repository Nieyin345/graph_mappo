...
**★ 2026-09-24 补：同协议下的现役参考点**（15 种子，240 步）

| 策略 | 验证 SR |
|---|---|
| **v3 + 需求隔离（当前最优，10 次 PPO）** | **0.7565**（300–329 窗 0.8545） |
| v3（默认需求传播，10 次 PPO） | 0.7376 |
| `matched240_v2_cold`（RL 20 轮） | 0.7707（50 轮口径；预算不同勿与上行比） |
| 专家锚（现环境） | 0.7674 |

> ⚠ **0.7971 已作废**：那是一次评估脚本的窗口覆盖 bug（验证环境建好后被重新
> 合并回训练窗口 0–295）。修正窗口后该 BC checkpoint 实为 **0.7490**。
> 「BC 不是越久越好」的结论方向不变（ep32 优于 ep60，12/15 下降），但引用数字
> 一律用 0.7490。详见 `docs/v3注意力评分诊断_20260924.md`。
>
> 这几个数**是配置相关的**：只在各自协议下可比，换环境或换 config 就要重测。
> 现役最优与专家的同协议配对比较**尚未做**（见 `docs/当前状态与优化方向.md` §3 P0）。
