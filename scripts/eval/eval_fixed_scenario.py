...
Usage:
    python scripts/eval/eval_fixed_scenario.py \
...
def main() -> None:
    ...
    validation_cfg = deep_merge(base, load_config([ROOT / args.config]))
    validation_cfg["train"]["rollout_steps"] = args.steps
    validation_cfg["env"]["episode_steps"] = args.steps
    data = load_checkpoint(args.checkpoint, device=args.device)
    cfg = checkpoint_validation_config(data.config, validation_cfg)
