...
OUT = REPO / "outputs"
LOGDIR = Path("/tmp/s5logs")

SEEDS = (42, 43, 44, 45, 46)
ARMS = [f"storage5_s{s}" for s in SEEDS]
...
def main() -> int:
    ...
    print("=" * 96)
    print("storage5 波次：num_layers 3 -> 4")
    print("=" * 96)
    print(f"  臂      {' '.join(ARMS)}")
    print(f"  对照    {' '.join(CTRL)}（同配置、同 BC 起点、同种子、u30 已跑完）")
    print(f"  configs {' '.join(CONFIGS)}")
    print(f"  唯一变量 reward.storage_reward_weight: 对照=0.5（缺省） / 臂=5.0\n  本波**同时起对照**（ent01_rerun 只有 s42..46，扩到 n=9 需补）")
    ...
    try:
        ...
        ctrl = arm = None
    EXPECTED = {"reward.storage_reward_weight": 5.0}
    if ctrl and arm:
