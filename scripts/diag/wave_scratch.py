...
def main() -> int:
    ...
    print("=" * 96)
    print("判决性对照：**无 BC 起点**（从零训）")
    print("=" * 96)
    print(f"  臂      {' '.join(ARMS)}")
    print(f"  对照    {' '.join(CTRL)}（有 BC 起点，已跑完 u30）")
    print(f"  configs {' '.join(CONFIGS)}")
    print(f"  唯一变量 --checkpoint: 臂=不给(从零) / 对照=supervised_pg_phased_latest.pt")
    print()
    ...
    # ---------- 5. 唯一变量自证：build_config 逐字段比 ----------
    print("\n  唯一变量核对（走 build_config()，不读 yaml）")
    try:
        ctrl = cfg_for(CONFIGS, "probe_ctrl", checkpoint=str(ck))
    ...
    if ctrl and arm:
        ...
        print(f"    基线字段数 {len(ctrl)}；差异字段 {len(diffs)} 个")
        for k, (a, b) in sorted(diffs.items()):
            print(f"      {k}: {a} → {b}")
        if diffs:
            print("    （预期为空：--checkpoint 是**命令行参数**，不进 build_config 的产物）")
