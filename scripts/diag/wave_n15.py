...
OUT = REPO / "outputs"
LOGDIR = Path("/tmp/n15logs")
TRAIN_PATTERN = "train_graph_mappo"

# 目标：ent01_rerun 补到 n=15（现有 42..46，新起 47..56）
SEEDS = tuple(range(51, 57))  # s42..s50 已存在（9 条），补 6 条到 n=15
ARMS = [f"ent01_rerun_s{s}" for s in SEEDS]
...
def main() -> int:
    ...
    print("=" * 96)
    print("补对照臂到 n=15：ent01_rerun_s51..s56")
    print("=" * 96)
    print(f"  新起 {len(ARMS)} 条（现有 42..50 共 9 条 ⟹ 补足 n=15）")
