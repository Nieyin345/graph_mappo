...
OUT = REPO / "outputs"
LOGDIR = Path("/tmp/nglogs")

SEEDS = (42, 43, 44, 45, 46)  # 对照 ent01_rerun_s42..46 已存在（u30 完成）
ARMS = [f"nograph_s{s}" for s in SEEDS]
# @@ 2026-09-21：对照 ent01_rerun **只有 s42..46** ⟹ 扩到 n=9 必须**同时起对照**，
#    否则 s47..50 没有配对对象。CTRL 在本波里也要真起。
CONTROL_ARMS = []  # @@ 对照已存在，不重起
...
def main() -> int:
    ...
    try:
        ...
        ctrl = arm = None
    EXPECTED = {"model.encoder.num_layers": 0}
    if ctrl and arm:
