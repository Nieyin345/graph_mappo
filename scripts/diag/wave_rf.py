...
OUT = REPO / "outputs"
LOGDIR = Path("/tmp/rflogs")

SEEDS = (42, 43, 44, 45, 46)
ARMS = [f"residfalse_s{s}" for s in SEEDS]
# @@ 2026-09-21：对照 ent01_rerun **只有 s42..46** ⟹ 扩到 n=9 必须**同时起对照**，
#    否则 s47..50 没有配对对象。CTRL 在本波里也要真起。
CONTROL_ARMS = []  # @@ 对照 ent01_rerun_s42..46 已存在（u30 已跑完），不重起
...
def main() -> int:
    ...
    try:
        ...
        ctrl = arm = None
    EXPECTED = {"model.encoder.residual": False}
    if ctrl and arm:
    ...
    for arm, seed, cfgs in jobs:
        ...
        # ★ `ulimit -n 65536`：ssh 会话默认软限制只有 1024，而一条臂要 spawn
        #    8 个 rollout worker，会在 resource_sharer.send_handle 处
        #    `Too many open files` 崩掉（2026-09-21 实测）。硬限制 1048576 够。
