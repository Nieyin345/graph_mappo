...
def fs_runs(window_s=900):
    ...
    ⚠⚠ 2026-09-20 实测的**非对称性**（第一版把链空转了 8 分钟）：
    `live_runs()` 只认 `train_graph_mappo.py`（真训练进程），而本函数按
    `metrics.jsonl` 的 mtime 数 ⟹ **一个 run 刚结束、进程已退出、但其
    metrics 的 mtime 还在 window 内时，本函数会把它继续算成"在跑"**。
    本波实测：ent001×3 + mini512×1 跑完退场后，
    `/proc 数到 3 条 vs 文件系统数到 6 条`，链因此**永远拒绝启动**。

    ⟹ **两条读数的偏差不是对称的**：
      · `fs > live`（本情形）= 刚死的 run 还挂在 mtime 窗口里 ⟹ **无害**，
        它只让交叉验证更保守（这边的 run 已终止，不可能造成 OOM）
      · `fs < live` = 有真训练进程没在写 metrics ⟹ **危险**，可能是卡死
    ⟹ 所以判据必须是**单向**的：只拒绝 `fs < live`，容忍 `fs > live`。
    "取绝对值差"在这里是错的——它把一个良性的保守偏差当成了故障。
    """
    now, out = time.time(), []
...
def main():
    ...
    while len(launched) < len(QUEUE) and time.time() - t0 < 5 * 3600:
        ...
        # ★ 独立读数交叉验证：**单向**判据（见 fs_runs 的 docstring）。
        #   fs < live ⟹ 有真训练进程没在写 metrics ⟹ 危险，拒绝启动并吵。
        #   fs > live ⟹ 只是刚结束的 run 还在 mtime 窗口里 ⟹ 无害，放行。
        if len(fs) < len(live_runs()):
            log("  !! 文件系统数到 %d 条 < /proc 数到 %d 条"
                " ⟹ 有训练进程没在写 metrics，门的输入不可信，**拒绝启动**"
                % (len(fs), len(live_runs())))
            time.sleep(120)
            continue
        if len(fs) > len(live_runs()):
            log("  · 文件系统多出 %d 条（刚结束的 run 仍在 mtime 窗口内）→ 无害，放行"
                % (len(fs) - len(live_runs())))
        if ok:
