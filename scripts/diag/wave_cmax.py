...
OUT = REPO / "outputs"
LOGDIR = Path("/tmp/cmaxlogs")

SEEDS = (42, 43, 44, 45, 46)
ARMS = [f"cmax_s{s}" for s in SEEDS]
...
def main() -> int:
    ...
    print("=" * 96)
    print("cmax 波次：critic 加一路 max 池化")
    print("=" * 96)
    print(f"  臂      {' '.join(ARMS)}")
    print(f"  对照    {' '.join(CTRL)}（同配置、同 BC 起点、同种子、u30 已跑完）")
    print(f"  configs {' '.join(CONFIGS)}")
    print(f"  唯一变量 model.critic.pool_include_max: 对照=false（缺省） / 臂=true")
    ...
    try:
        ...
        ctrl = arm = None
    EXPECTED = {"model.critic.pool_include_max": True}
    if ctrl and arm:
