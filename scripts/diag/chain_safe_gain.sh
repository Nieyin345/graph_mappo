...
launch() {     # $1=run-name  $2=seed  $3...=额外 config（**裸文件名**，不带 configs/）
  local name="$1"; local seed="$2"; shift 2
  # --configs 的每一项会被解析成 <ROOT>/configs/<name>（train_graph_mappo.py:103），
  # 所以这里必须传裸文件名。上一版传了 "configs/xxx.yaml"，
  # 于是拼成 configs/configs/xxx.yaml → FileNotFoundError。
  # 训练脚本**只在启动时**才报这个错，而启动要走 ssh + 等内存，
  # 所以这个错值得在调用前就挡住。
  for c in "$@"; do
    case "$c" in
...
wave vcoef1 train_safe_vcoef1.yaml
if [ "$ONLY1" -eq 1 ]; then
  echo "[$(date -Is)] --wave1-only：链条交回本地唤醒链接续"
  exit 0
fi

wave ep2     train_safe_ep2.yaml
wave mini512 train_safe_mini512.yaml
