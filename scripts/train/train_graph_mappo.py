...
Usage (from the project root):
    ...
    conda run -n pytorch python scripts/train/train_graph_mappo.py
    conda run -n pytorch python scripts/train/train_graph_mappo.py --num-updates 100 --run-name exp1
    conda run -n pytorch python scripts/train/train_graph_mappo.py --seed 7
    conda run -n pytorch python scripts/train/train_graph_mappo.py --checkpoint outputs/exp1/checkpoint_update_0100.pt
"""
...
# CPU thread budget: honour OMP_NUM_THREADS/MKL_NUM_THREADS when the caller set
# them, because every experiment protocol in docs/测试规范.md pins the thread
# count and the results are NOT comparable across different thread counts.
# Only fall back to cpu_count()//2 when nothing was pinned.
_pinned = os.environ.get("OMP_NUM_THREADS") or os.environ.get("MKL_NUM_THREADS")
_cpu_threads = int(_pinned) if _pinned and _pinned.isdigit() else max(1, (os.cpu_count() or 4) // 2)
