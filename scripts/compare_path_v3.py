"""Compare Path-Greedy (request-level path activation) vs V3 (edge-level
scoring) vs Random on the same deterministic real env episode.

The ONLY difference is how links are activated; env / request stream identical.
"""
...
def run(profile, seed, start_seed, steps, policy_type, w=None, trace=False):
...
def main():
    ap = argparse.ArgumentParser(description="Path-Greedy vs V3 vs Random.")
    ...
    pg = run(profile, args.seed, args.start_seed, args.steps, "pathgreedy", trace=True)
    ...
    print(f"PathGreedy SR = {pg[0]:.4f} (served={pg[1]:,.0f}, activ/slot={pg[2]:.1f})")
    print(f"V3          SR = {v3[0]:.4f} (served={v3[1]:,.0f}, activ/slot={v3[2]:.1f})")
    print(f"delta(PG-V3)  = {pg[0]-v3[0]:+.4f}")
